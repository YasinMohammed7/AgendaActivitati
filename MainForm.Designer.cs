namespace AgendaActivitati
{
    partial class MainForm
    {
        /// <summary>
        ///  Required designer variable.
        /// </summary>
        private System.ComponentModel.IContainer components = null;

        /// <summary>
        ///  Clean up any resources being used.
        /// </summary>
        /// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
        protected override void Dispose(bool disposing)
        {
            if (disposing && (components != null))
            {
                components.Dispose();
            }
            base.Dispose(disposing);
        }

        #region Windows Form Designer generated code

        /// <summary>
        ///  Required method for Designer support - do not modify
        ///  the contents of this method with the code editor.
        /// </summary>
        private void InitializeComponent()
        {
            activityButton = new Button();
            SuspendLayout();
            // 
            // activityButton
            // 
            activityButton.BackColor = Color.Green;
            activityButton.DialogResult = DialogResult.Yes;
            activityButton.ForeColor = Color.White;
            activityButton.Location = new Point(590, 415);
            activityButton.Name = "activityButton";
            activityButton.Size = new Size(113, 23);
            activityButton.TabIndex = 0;
            activityButton.Text = "Adauga activitate";
            activityButton.UseVisualStyleBackColor = false;
            activityButton.Click += activityButton_Click;
            // 
            // MainForm
            // 
            AutoScaleDimensions = new SizeF(7F, 15F);
            AutoScaleMode = AutoScaleMode.Font;
            ClientSize = new Size(800, 450);
            Controls.Add(activityButton);
            Name = "MainForm";
            Text = "Form1";
            ResumeLayout(false);
        }

        #endregion

        private Button activityButton;
    }
}
